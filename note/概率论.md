...
$$
$$
P\left(-u_{\alpha/2} < \frac{\overline{X} - \mu}{\sigma/\sqrt{n}} < u_{\alpha/2}\right) = 1 - \alpha
$$
$$
\Rightarrow P\left(\overline{X} - \frac{\sigma}{\sqrt{n}} u_{\alpha/2} < \mu < \overline{X} + \frac{\sigma}{\sqrt{n}} u_{\alpha/2}\right) = 1 - \alpha
$$

注：
a. 给定 $\alpha$，$n$ 越大，$l$ 越小；
b. 给定 $n$，$\alpha$ 越小，$l$ 越大。
...
#### 1. $\mu$ 已知
$(X_1, X_2, \dots, X_n)$ 是总体 $N(\mu, \sigma^2)$ 的样本。
