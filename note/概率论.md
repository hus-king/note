...
① $F_{\max}(x)=P\{\max\{X_1,X_2,\cdots,X_n\}\leq x\}=F_{X_1}(x)\cdots F_{X_n}(x)$

② $F_{\min}(x)=P\{\min\{X_1,X_2,\cdots,X_n\}\leq x\}=1 - P\{\min\{X_1,X_2,\cdots,X_n\}\gt x\}$
...
④ $X\sim$ 超几何分布，$EX = n\frac{M}{N}$

⑤ $X\sim$ 几何分布，$EX=\frac{1}{p}$
...
① R.V.X $E(X)=\mu$，$D(X)=\sigma^{2}$，$X$的标准化 $Y=\frac{X - \mu}{\sigma}$，$E(Y)=0$，$D(Y)=1$

② 设$X_{i}(i = 1,2,\cdots,n)$独立同分布 (i.i.d.)，且$E(X_{i})=\mu$，$D(X_{i})=\sigma^{2}$
...
**例**：$X\sim N(\mu,\sigma^{2})$，则$E((X - \mu)^{k})=\begin{cases}0 & k为奇数\\\sigma^{k}(k - 1)!! & k为偶数\end{cases}$

## 4.4 协方差与相关系数
### 4.4.1 协方差
1. **定义**：$Cov(X,Y)=E((X - EX)(Y - EY))$

注：① $Cov(X,Y)=E(XY)-(EX)(EY)$

② $Cov(X,Y)=0\Leftrightarrow X,Y$不相关
...
② $|\rho_{XY}| = 1\Leftrightarrow$存在常数$a,b$使$P(Y = a + bX)=1$
