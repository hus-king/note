...
### 3. 数据项（Data Item）
是构成数据元素的最小单位，是对数据元素某些特性的具体描述。

### 4. 数据对象（Data Object）
是性质相同的数据元素的集合，是数据的一个子集。

### 5. 逻辑结构（Logical Structure）
是从逻辑关系上描述数据，与数据在计算机中的存储方式无关。包括：
- 集合结构
- 线性结构
- 树形结构
- 图状结构

### 6. 存储结构（Storage Structure） / 物理结构
是数据结构在计算机中的表示或实现方式，主要有两种：
- 顺序存储：用一组连续的存储单元依次存放数据元素。
- 链式存储：用指针将数据元素链接起来。
...
### 8. 算法的四大设计目标
- 正确性（Correctness）：算法能正确解决问题。
- 可读性（Readability）：便于阅读和理解。
- 健壮性（Robustness）：对非法输入有合理的反应。
- 高效率与低存储量需求（Efficiency and Low Memory Usage）：时间复杂度和空间复杂度要小。
...
### 1. 线性表（Linear List）
是由n(n≥0)个数据元素组成的有限序列，具有唯一前驱和后继的关系。

### 2. 顺序表（Sequential List）
是线性表的顺序存储结构，使用数组实现，逻辑相邻的元素物理位置也相邻。

### 3. 单链表（Singly Linked List）
是线性表的链式存储结构，每个节点包含数据域和指向下一个节点的指针域。

### 4. 静态链表（Static Linked List）
用数组模拟链表，通过数组下标代替指针来实现链式结构。
...
### 1. 栈（Stack）
是一种只能在一端进行插入和删除的线性表，遵循“**后进先出**”原则（LIFO）。

### 2. 队列（Queue）
是一种只能在队尾插入、队头删除的线性表，遵循“**先进先出**”原则（FIFO）。

### 3. 后缀表达式（Postfix Expression）
又称逆波兰表达式，运算符位于操作数之后，无需括号即可明确运算顺序，常用于计算器实现。
...
图的一种链式存储结构，为每个顶点建立一个单链表，表示与该顶点相连的其他顶点。

### 2. 十字链表（Orthogonal List）
是有向图的链式存储结构，结合了邻接表和逆邻接表的优点，方便查找入边和出边。

### 3. 邻接多重表（Adjacency Multilist）
是无向图的链式存储结构，每条边只存储一次，便于共享边信息。

### 4. Prim算法
是一种求解**最小生成树**的贪心算法，从一个顶点出发逐步扩展生成树。

### 5. Kruskal算法
也是一种求解**最小生成树**的贪心算法，按边权从小到大选择边构造生成树。

### 6. Dijkstra算法
是一种求解**单源最短路径**的经典算法，适用于边权非负的图。

### 7. Floyd算法
是一种求解**所有顶点对之间的最短路径**的动态规划算法。

### 8. DAG图拓扑排序（Topological Sort）
对有向无环图（DAG）进行顶点排序，使得对于每一条有向边(u, v)，u排在v之前。

### 9. AOE网相关概念
AOE网（Activity On Edge Network）是以边表示活动的有向无环图，用于估算工程完成时间：

- **最早发生时间 ve(v)**：顶点v的最早开始时间。
- **最晚允许发生时间 vl(v)**：在不推迟整个工期的前提下，顶点v的最晚开始时间。
- **活动的余量（松弛时间）**：某个活动的最晚开始时间减去最早开始时间。
- **关键路径**：从起点到终点的最长路径，决定了整个工程的最短完成时间。

### 10. 并查集（Union-Find Set）
是一种树型数据结构，用于高效管理元素的分组情况，支持合并（Union）和查找（Find）操作。
...
将待排序元素逐个插入已排序序列的合适位置。

#### a. 直接插入排序
每次将一个记录插入到前面已排序好的序列中。

#### b. 折半插入排序
利用折半查找确定插入位置，减少比较次数。

#### c. 希尔排序（Shell Sort）
是插入排序的改进版本，通过缩小增量分组排序提高效率。
...
#### a. 冒泡排序（Bubble Sort）
两两比较相邻元素，若顺序错误则交换，重复遍历直到有序。

#### b. 快速排序（Quick Sort）
采用分治策略，选取基准值将数组划分为两个子数组，递归排序。

---

### 3. 选择排序（Selection Sort）
每次选择最小（或最大）元素放到当前未排序部分的起始位置。

#### a. 简单选择排序
每趟从未排序部分选出最小元素，与第一个元素交换。

#### b. 树状选择排序
构建一棵选择树，加快最小元素的选择过程。

#### c. 堆排序（Heap Sort）
利用堆这种特殊完全二叉树结构进行排序，效率较高。

---

### 4. 归并排序（Merge Sort）
采用分治策略，将数据分成若干子序列分别排序后再合并成一个有序序列。
...
#### a. 桶排序（Bucket Sort）
将数据分配到有限数量的桶中，每个桶再分别排序。

#### b. 基数排序（Radix Sort）
按位数从低位到高位依次进行排序，属于稳定排序，适用于整数或字符串排序。
...
void merge(int *a, int l, int m, int r) {
    int n1 = m - l + 1, n2 = r - m;
    int *L = malloc((n1 + n2) * sizeof(int)), *R = L + n1;
