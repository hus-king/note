...
假设二叉树有 $n$ 个结点。估计上述算法的时间复杂度为 $O(n)$，因为先访问根的左右子树结点均均衡分布的特殊情况，$n$ 充分大时如下图所示：

![图示](#) <!-- 图片占位符 -->

可得 $T(n) = O(n)$。另外，当二叉树为一般情况时，我们也可以分析推导同样的结果。递归处理需要利用系统栈，计算的空间复杂度在最坏与最好情况下分别为 $O(\log n), O(n)$。
